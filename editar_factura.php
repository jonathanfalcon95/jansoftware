...
date_default_timezone_set('America/Caracas');
	...
	if (isset($_GET['id_factura']))
	{
		...
		$campos="clientes.id_cliente, clientes.nombre_cliente, clientes.telefono_cliente, clientes.cedula, facturas.id_vendedor, facturas.fecha_factura, facturas.condiciones, facturas.estado_factura, facturas.numero_factura";
		...
		if ($count==1)
		{
				...
				$cedula_cliente=$rw_factura['cedula'];
...
<html lang="en">
  ...
  <body>
		...
		?>
			<form class="form-horizontal" role="form" id="datos_factura">
				<div class="form-group row">
                            ...
                            <div class="col-md-2">
                                <input type="text" class="form-control input-sm" id="cedula1"   placeholder="Ingrese la cedula" value="<?php echo $cedula_cliente;?>">

                            </div>
                            <div class="col-md-1">
                                <button type="button" class="btn btn-default btn-sm"onclick="buscarcedula();" >
                                    <span   class="glyphicon glyphicon-search"></span>Buscar
                                </button>
                            </div>
				  <label for="nombre_cliente" class="col-md-1 control-label">Cliente</label>
				  <div class="col-md-3">
                                      <input type="text" class="form-control input-sm" id="nombre_cliente"  disabled="" placeholder=" cliente" required value="<?php echo $nombre_cliente;?>">
	...
	<script src="js/jquery.js"></script>
        ...
        <!--<script src="js/jquery-ui.min.css.js"></script>-->
...
//            });
                                    ...
                                    function buscarcedula() {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            dataType: "json",
                                            url: "ajax/buscar_clientes1.php?action=ajax&cedula=" + cedula,
                                        }).done(function(respuesta) {
                                            if (respuesta[0]['validacion'] === "no") {
                                                if (confirm("Cliente no encontrado ¿Desea incluirlo?")){
                                                      $("#nuevoCliente").modal("show");
                                                      $("#nuevoCliente #cedula").val(cedula);
                                                ...
                                                var id = respuesta[0]['id_cliente'];
                                                var nombre = respuesta[0]['nombre_cliente'];
                                                var telefono = respuesta[0]['telefono_cliente'];
                                                $('#id_cliente').val(id);
